...
APP_VERSION = "v2.4.5"
LAST_UPDATED = "Dec 23, 2025 - 2:35 PM PST - Fix all dropdowns"
...
# Use selectbox WITH first item as placeholder
applicant_type = st.selectbox(
    "Applicant type",
    ["Select...", "First Nation", "Indigenous organization", "Municipality / Regional District", "Non-profit / Charity", "For-profit business", "University / Research institute", "Other"],
    index=0
)
...
st.markdown('<div class="section-header"><div class="section-number">2</div><div><h3>Project basics</h3><p class="section-sub">💡 Mention funders (SFI, HCTF) to boost ranking +25 pts</p></div></div><div class="input-section">', unsafe_allow_html=True)
...
description = st.text_area("Description", height=120, placeholder="Describe your project. Mention funders (SFI, HCTF) for better matches...")
st.markdown("</div><hr>", unsafe_allow_html=True)

def render_matches(df):
    st.markdown('<div class="section-header"><div class="section-number">3</div><div><h3>Your matches</h3><p class="section-sub">Keyword matches earn up to +25 pts</p></div></div>', unsafe_allow_html=True)
...
if st.button("🔍 Find funding matches", type="primary", use_container_width=True):
    ...
    st.session_state['user_intake'] = {"organization": org_name, "name": name.strip(), "email": email.strip(), "applicant_type": applicant_type, "region": region, "budget_range": budget_range, "project_types": project_types, "themes": themes, "stage": stage, "project_title": project_title, "description": description, "partners": partners}
    submission_fields = {"Organization": org_name if org_name else f"{applicant_type} Org", "Name": name.strip(), "Email": email.strip(), "Applicant Type": applicant_type, "Region": region if region else "BC", "Budget Range": budget_range, "Project Types": ", ".join(project_types) if project_types else "", "Project Title": project_title if project_title else "Project", "Description": description if description else "", "Stage": stage, "Themes": ", ".join(themes) if themes else "", "Partners": partners if partners else ""}
    submission_id = create_project_submission(submission_fields)
    if submission_id:
        st.session_state['submission_id'] = submission_id
        st.success(f"✅ Saved: {name.strip()} ({email.strip()})")
    else:
        st.stop()
