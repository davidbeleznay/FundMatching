...
APP_VERSION = "v2.5.0"
LAST_UPDATED = "Dec 23, 2025 - 2:45 PM PST - Fix autofill + dropdowns"
...
def update_project_submission(record_id: str, fields: dict) -> bool:
    ...
    return requests.patch(url, headers=AIRTABLE_HEADERS, json={"fields": fields}).status_code == 200
        ...
        if len(program_words) >= 3:
            for i in range(len(program_words) - 2):
                ...
                if len(phrase) > 12 and phrase in user_text_lower:
                    score += 12
                    break
        for term in ["climate smart", "habitat conservation", "watershed security", "salmon resiliency", "freshwater ecosystem"]:
...
st.markdown('<div class="hero"><p class="eyebrow">Funding Matcher</p><h1>EcoProject Navigator</h1><p style="color:var(--muted);margin-top:8px;">Match BC environmental projects to funding opportunities</p><div class="pill" style="margin-top:12px;"><span class="dot"></span>Smart keyword matching · Deep AI analysis</div></div>', unsafe_allow_html=True)

st.markdown('<div class="section-header"><div class="section-number">1</div><div><h3>Who are you?</h3><p class="section-sub">Required for matching and Deep Dive delivery</p></div></div><div class="input-section">', unsafe_allow_html=True)

# Store values in session state on change to detect autofill
...
name_input = st.text_input("Your name", placeholder="e.g., Sarah Johnson", value=st.session_state.form_name, key="name_field")
email_input = st.text_input("Email", placeholder="e.g., sarah@example.com", value=st.session_state.form_email, key="email_field")
...
applicant_type = st.selectbox("Applicant type", ["Select...", "First Nation", "Indigenous organization", "Municipality / Regional District", "Non-profit / Charity", "For-profit business", "University / Research institute", "Other"], index=0)
partners = st.text_input("Partners (optional)", placeholder="e.g., Mosaic Forest Management")
st.markdown("</div>", unsafe_allow_html=True)

st.markdown('<div class="section-header"><div class="section-number">2</div><div><h3>Project basics</h3><p class="section-sub">💡 Mention programs (SFI, HCTF) for +25 point boost</p></div></div><div class="input-section">', unsafe_allow_html=True)
...
with col1:
    ...
    stage = st.selectbox("Stage", ["Idea", "Planning", "Ready to implement", "Shovel-ready"], index=1)
with col2:
...
description = st.text_area("Description", height=120, placeholder="Describe project. Mention funders (SFI, HCTF) for better matches...")
st.markdown("</div><hr>", unsafe_allow_html=True)

def render_matches(df):
    st.markdown('<div class="section-header"><div class="section-number">3</div><div><h3>Your matches</h3><p class="section-sub">Keyword matches: up to +25 pts</p></div></div>', unsafe_allow_html=True)
    ...
    for idx, row in df.iterrows():
        ...
        st.markdown(f'<div class="program-top"><div><p class="eyebrow">Funding</p><h3>🐟 {program_name}{keyword_badge}</h3></div><div class="score-badge"><span style="font-size:1.4rem;">{int(row["Score"])}</span><small style="margin-left:4px;">fit</small></div></div>', unsafe_allow_html=True)
        ...
        with c1:
            if st.button("💧 Deep Dive", key=f"dd_{idx}", use_container_width=True):
                if submission_id and trigger_deep_dive(submission_id, row.get("id"), program_name):
                    ...
                    st.info(f'📧 Email within 24h to **{st.session_state["user_intake"].get("email")}**\n\n**{program_name}**\n✓ Verdict ✓ Actions ✓ Fit ✓ Docs ✓ Budget ✓ Flags')
...
if st.button("🔍 Find funding matches", type="primary", use_container_width=True):
    # Use session state values which capture autofill
    final_name = st.session_state.form_name.strip()
    final_email = st.session_state.form_email.strip()
