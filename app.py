...
APP_VERSION = "v2.4.2"
LAST_UPDATED = "Dec 23, 2025 - 7:15 AM PST - Cache refresh for new programs"
...
def load_funding_programs() -> pd.DataFrame:
    """Load all funding programs from Airtable with pagination"""
    ...
    while True:
        ...
        if resp.status_code != 200:
            st.error(f"Error loading programs (page {page_count}): {resp.status_code}")
            break
        ...
        # Safety: max 10 pages (500 records)
        if page_count > 10:
            st.warning("⚠️ Stopped at 500 records (pagination limit)")
            break
    
    df = pd.DataFrame(all_records)
    
    # Show count in sidebar for debugging
    if not df.empty:
        st.sidebar.info(f"📊 Loaded {len(df)} programs from Airtable")
...
def raw_score_program(row, applicant_type, project_types, themes, budget_range, region, stage, project_title, description, partners):
    ...
    s[0] = 10 if not region_norm else (15 if not elig_regions else (25 if any(region_norm in r.lower() or r.lower() in region_norm for r in elig_regions) else 0))
...
description = st.text_area("Description", height=120, placeholder="Describe your project. Mention specific funders if known (e.g., 'Applying to SFI program')...")
        ...
        with c1:
            if st.button("💧 Request Deep Dive", key=f"dd_{idx}", use_container_width=True):
                if submission_id and trigger_deep_dive(submission_id, row.get("id"), program_name):
                    ...
                    st.info(f'📧 **Detailed report within 24 hours**\n\nAnalyzing: **{program_name}**\nRecipient: **{st.session_state["user_intake"].get("email")}**\n\n✓ GO/NO-GO verdict\n✓ Strategic actions\n✓ Fit analysis\n✓ Required documents\n✓ Budget guidance\n✓ Red flags')
                    st.markdown("""<script>const s=document.createElement('style');s.textContent='@keyframes bubble{0%{bottom:-50px;opacity:1}100%{bottom:100vh;opacity:0}}.bubble{position:fixed;background:radial-gradient(circle,#5eead4,#14b8a6);border-radius:50%;animation:bubble 5s ease-in infinite;z-index:9999;pointer-events:none}';document.head.appendChild(s);for(let i=0;i<12;i++){const b=document.createElement('div');b.className='bubble';const sz=Math.random()*15+8;b.style.width=b.style.height=sz+'px';b.style.left=Math.random()*100+'%';b.style.animationDelay=Math.random()*2+'s';b.style.animationDuration=(Math.random()*2+4)+'s';document.body.appendChild(b);setTimeout(()=>b.remove(),6000)}</script>""", unsafe_allow_html=True)
                else:
...
if st.button("🔍 Find funding matches", type="primary", use_container_width=True):
    ...
    submission_fields = {"Organization": org_name if org_name else f"{applicant_type} Organization", "Name": name, "Email": email, "Applicant Type": applicant_type, "Region": region if region else "BC", "Budget Range": budget_range, "Project Types": ", ".join(project_types) if project_types else "", "Project Title": project_title if project_title else "Untitled Project", "Description": description if description else "", "Stage": stage, "Themes": ", ".join(themes) if themes else "", "Partners": partners if partners else ""}
