...
APP_VERSION = "v2.6.0"
LAST_UPDATED = "Dec 24, 2025 - 3:00 PM PST - Add hero background"
...
st.markdown('<div class="hero"><p class="eyebrow">BC Environmental Funding</p><h1>🌲 EcoProject Navigator</h1><p style="color:var(--muted);margin-top:8px;font-size:1.1rem;text-shadow:0 1px 10px rgba(0,0,0,0.6);">Match your project to funding opportunities</p><div class="pill" style="margin-top:16px;"><span class="dot"></span>Smart keyword matching · Deep analysis</div></div>', unsafe_allow_html=True)
        ...
        with c1:
            if st.button("💧 Deep Dive", key=f"dd_{idx}", use_container_width=True):
                if submission_id and trigger_deep_dive(submission_id, row.get("id"), program_name):
                    ...
                    st.info(f'📧 Email to **{st.session_state["user_intake"].get("email")}**\n\n**{program_name}**\n✓ Verdict ✓ Actions ✓ Fit ✓ Docs ✓ Budget')
