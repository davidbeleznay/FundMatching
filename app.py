...
# VERSION TRACKING
APP_VERSION = "v2.3.0"
LAST_UPDATED = "Dec 22, 2025 - 8:10 PM PST - Lighter UI + Snow"
...
def create_project_submission(fields: dict) -> str | None:
    ...
    clean_fields = {key: (", ".join(str(v) for v in value) if isinstance(value, list) else value) for key, value in fields.items()}
    resp = requests.post(url, headers=AIRTABLE_HEADERS, json={"fields": clean_fields})
    if resp.status_code == 200:
        return resp.json().get("id")
    st.warning("⚠️ Could not save")
...
def update_project_submission(record_id: str, fields: dict) -> bool:
    ...
    if resp.status_code != 200:
        st.error(f"Update error: {resp.status_code}")
...
def raw_score_program(row, applicant_type, project_types, themes, budget_range, region, stage):
    s = [0, 0, 0, 0, 0, 0]  # region, applicant, type, themes, budget, stage
    ...
    proj_types = {pt.lower() for pt in project_types} if project_types else set()
    if proj_types and elig_types:
        overlap = proj_types & elig_types
        s[2] = int(20 * min(1.0, len(overlap) / len(proj_types))) if overlap else 0
    elif not elig_types:
        s[2] = 10
    prog_themes = {t.lower() for t in as_list(row.get("Themes") or row.get("Focus_Themes") or row.get("Eligible_Themes"))}
    user_themes = {t.lower() for t in themes} if themes else set()
    if user_themes and prog_themes:
        overlap = user_themes & prog_themes
        s[3] = int(10 * min(1.0, len(overlap) / len(user_themes))) if overlap else 0
...
st.markdown('<div class="hero"><p class="eyebrow">Funding Matcher</p><h1>EcoProject Navigator</h1><p style="color:var(--muted);margin-top:8px;">Match your project to funding with AI insights</p><div class="pill" style="margin-top:12px;"><span class="dot"></span>48+ programs · Deep Dive · Grant Readiness</div></div>', unsafe_allow_html=True)
...
def render_matches(df):
    st.markdown('<div class="section-header"><div class="section-number">3</div><div><h3>Your matches</h3><p class="section-sub">Ranked by score</p></div></div>', unsafe_allow_html=True)
        ...
        with c1:
            if st.button("🔍 Deep Dive", key=f"dd_{idx}", use_container_width=True):
                if submission_id and trigger_deep_dive(submission_id, row.get("id"), program_name):
                    ...
                    st.info(f'📧 **Check email within 24 hours**\n\nAnalyzing: **{program_name}**\nSent to: **{st.session_state["user_intake"].get("email")}**\n\n✓ GO/NO-GO score\n✓ Action items\n✓ Fit analysis\n✓ Documents needed\n✓ Budget strategy\n✓ Red flags')
                    st.snow()
                else:
...
if st.button("🔍 Find funding matches", type="primary", use_container_width=True):
    ...
    submission_id = create_project_submission({"Name": name, "Email": email, "Applicant Type": applicant_type, "Region": region, "Budget Range": budget_range, "Project Types": ", ".join(project_types) if project_types else "", "Project Title": project_title, "Description": description, "Stage": stage, "Themes": ", ".join(themes) if themes else "", "Partners": partners})
    if submission_id:
