...
APP_VERSION = "v2.4.1"
LAST_UPDATED = "Dec 23, 2025 - 6:50 AM PST - Fix dropdown styling + autofill detection"
...
def check_keyword_match(user_text: str, program_name: str, funder_name: str) -> int:
    """Check if user mentions program/funder by name. Returns 0-15 bonus points"""
    if not user_text:
        return 0
    ...
    # Check program name keywords
    if program_name:
        program_lower = program_name.lower()
        
        # Check for acronyms (SFI, HCTF, BCSRIF, etc)
        words = program_name.split()
        for word in words:
            if len(word) <= 6 and word.isupper():  # Likely acronym
                if word.lower() in user_text_lower:
                    score += 10  # Big bonus for acronym match
                    break
        
        # Check for distinctive multi-word phrases (3+ words)
        ...
        # Check for key terms (climate smart, habitat conservation, etc)
        key_terms = ["climate smart", "habitat conservation", "watershed security", "environmental damages"]
        for term in key_terms:
            if term in program_lower and term in user_text_lower:
                score += 6
    
    # Check funder name
    if funder_name and len(funder_name) > 3:
        if funder_name.lower() in user_text_lower:
            score += 5
...
def raw_score_program(row, applicant_type, project_types, themes, budget_range, region, stage, project_title, description, partners):
    """
    Enhanced scoring v2.4
    Base (100): Region 25, Applicant 30, Type 20, Themes 15, Budget 10
    Bonuses (+30 max): Stage +5, Keyword +15, Deadline +3, Salmon +3, Indigenous +4
    """
    ...
    if region_norm:
        score_region = 12 if not elig_regions else (25 if any(region_norm in r.lower() or r.lower() in region_norm for r in elig_regions) else 0)
    else:
        score_region = 8
    ...
    score_applicant = 15 if not elig_apps else (30 if any(app_norm in a.lower() for a in elig_apps) else 0)
    ...
    score_budget = 5 if not proj_budget or not max_amt else (10 if proj_budget <= max_amt else (5 if proj_budget <= 1.5 * max_amt else 0))
    ...
    total = score_region + score_applicant + score_type + score_themes + score_budget + bonus_stage + bonus_keyword + bonus_deadline + bonus_salmon + bonus_indigenous
...
with st.sidebar:
    ...
    if st.button("Test HCTF", use_container_width=True):
        ...
        if not df.empty:
            ...
            if not hctf.empty:
                st.session_state.update({'user_intake': {"organization": "Test Group", "name": "Test User", "email": "test@example.com", "applicant_type": "Non-profit / Charity", "region": "Vancouver Island", "budget_range": "$50–250k", "project_types": ["Riparian planting"], "themes": ["Salmon habitat"], "stage": "Ready to implement", "project_title": "Salmon Restoration", "description": "Restore riparian", "partners": ""}, 'selected_program': hctf.iloc[0].to_dict(), 'page': 'grant_readiness'})
                st.rerun()

st.markdown('<div class="hero"><p class="eyebrow">Funding Matcher</p><h1>EcoProject Navigator</h1><p style="color:var(--muted);margin-top:8px;">Match your project to BC environmental funding with AI-powered insights</p><div class="pill" style="margin-top:12px;"><span class="dot"></span>Keyword matching · Deadline analysis · Indigenous priority</div></div>', unsafe_allow_html=True)
...
name = st.text_input("Your name", placeholder="e.g., Sarah Johnson", key="user_name")
email = st.text_input("Email", placeholder="e.g., sarah@example.com", key="user_email")
...
partners = st.text_input("Partners (optional)", placeholder="e.g., First Nation partners, Mosaic Forest Management")
st.markdown("</div>", unsafe_allow_html=True)

st.markdown('<div class="section-header"><div class="section-number">2</div><div><h3>Project basics</h3><p class="section-sub">💡 Tip: Mention specific programs (SFI, HCTF, BCSRIF) for smarter matching</p></div></div><div class="input-section">', unsafe_allow_html=True)
...
with col2:
    project_types = st.multiselect("Project type(s)", ["Culvert replacement", "Road deactivation / upgrades", "Riparian planting", "Instream LWD / channel work", "Forest restoration", "Planning / assessment", "Monitoring", "Community engagement / education", "Land acquisition / conservation"], placeholder="Select activities...")
    themes = st.multiselect("Themes", ["Climate adaptation", "Salmon habitat", "Watershed health", "Flood resilience", "Wildfire resilience", "Forest roads & access", "Erosion & sediment", "Water quality", "Biodiversity", "Wetlands & beavers", "Drinking water protection", "Community engagement / stewardship"], placeholder="Select themes...")
project_title = st.text_input("Project title", placeholder="e.g., Climate Smart Forestry with Indigenous Partners")
description = st.text_area("Description", height=120, placeholder="Describe your project goals and activities. Mention specific funders if you know them (e.g., 'Applying to SFI Climate Smart program')...")
    ...
    for idx, row in df.iterrows():
        ...
        with c1:
            if st.button("💧 Request Deep Dive", key=f"dd_{idx}", use_container_width=True):
                if submission_id and trigger_deep_dive(submission_id, row.get("id"), program_name):
                    ...
                    st.info(f'📧 **Detailed report coming to your email within 24 hours**\n\nAnalyzing: **{program_name}**\nRecipient: **{st.session_state["user_intake"].get("email")}**\n\n✓ GO/NO-GO verdict\n✓ Strategic actions\n✓ Fit analysis\n✓ Required documents\n✓ Budget guidance\n✓ Red flags')
                    st.markdown("""<script>const style=document.createElement('style');style.textContent='@keyframes bubble{0%{bottom:-50px;opacity:1}100%{bottom:100vh;opacity:0}}.bubble{position:fixed;background:radial-gradient(circle,#5eead4,#14b8a6);border-radius:50%;animation:bubble 5s ease-in infinite;z-index:9999;pointer-events:none}';document.head.appendChild(style);for(let i=0;i<12;i++){const b=document.createElement('div');b.className='bubble';const size=Math.random()*15+8;b.style.width=b.style.height=size+'px';b.style.left=Math.random()*100+'%';b.style.animationDelay=Math.random()*2+'s';b.style.animationDuration=(Math.random()*2+4)+'s';document.body.appendChild(b);setTimeout(()=>b.remove(),6000)}</script>""", unsafe_allow_html=True)
                else:
        ...
        with c2:
            if has_template(program_name):
                ...
                st.button("📋 Grant Readiness", key=f"gr_{idx}", disabled=True, help="Coming soon", use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    st.success(f"✅ {len(df)} programs matched!")

if st.button("🔍 Find funding matches", type="primary", use_container_width=True):
    # Check if fields have values (handles autofill)
